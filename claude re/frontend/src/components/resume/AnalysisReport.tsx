...
import {
  ...
  Award, TrendingUp, Copy, Check, Pencil
} from 'lucide-react';
...
Generated by RecruitIQ — recruitiq.ai
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      ...
      {/* Resume Rewrite — Pro Feature */}
      <Section title={`Resume Rewrite ${isPro ? '' : '— Pro Feature'}`} icon={<Pencil size={16} className="text-indigo-400" />} defaultOpen={false}>
        {!isPro ? (
          <div className="text-center py-6">
            <Lock size={28} className="text-zinc-600 mx-auto mb-3" />
            <p className="text-zinc-400 text-sm mb-1">Resume rewrite requires <strong className="text-white">Pro plan</strong></p>
              ...
              {rewriteData.rewrites?.map((r: any, i: number) => (
                <div key={i} className="rounded-xl overflow-hidden border border-white/8">
                  ...
                  <div className="p-3 bg-emerald-500/5">
                    ...
                    {r.improvement && <p className="text-zinc-600 text-xs mt-1.5">✦ {r.improvement}</p>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="text-center py-6 space-y-4">
            <div className="space-y-2">
              <Pencil size={28} className="text-indigo-400 mx-auto" />
              <p className="text-zinc-300 font-medium">AI Resume Rewriter</p>
              <p className="text-zinc-500 text-sm max-w-sm mx-auto">
                AI rewrites your weak bullet points with strong action verbs and metrics — showing before and after for each line.
              </p>
            </div>
            ...
            <button onClick={handleGenerateRewrite} disabled={loadingRewrite}
              className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 mx-auto transition-colors">
      ...
      {/* Interview Questions */}
      <Section title={`AI Interview Prep ${isPro ? '(4 Questions)' : '— Pro Feature'}`} icon={<MessageSquare size={16} className="text-violet-400" />} defaultOpen={false}>
        {!isPro ? (
          <div className="text-center py-6">
            <Lock size={28} className="text-zinc-600 mx-auto mb-3" />
            <p className="text-zinc-400 text-sm mb-1">Interview prep requires <strong className="text-white">Pro plan</strong></p>
            <p className="text-zinc-500 text-xs mb-4">Get 4 AI-tailored interview questions based on your resume</p>
            <button onClick={() => navigate('/pricing')} className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold rounded-xl transition-colors">
              Upgrade to Pro — ₹49
            </button>
          </div>
          ...
          <div className="space-y-4">
            {interviews.map((q: any, i: number) => (
              <div key={i} className="p-4 glass rounded-xl">
                ...
                {q.why_asked && <p className="text-zinc-500 text-xs mb-1"><span className="text-violet-400 font-medium">Why asked:</span> {q.why_asked}</p>}
          ...
          <div>
            {!showInterviewInput ? (
              <div className="text-center py-4">
                <p className="text-zinc-400 text-sm mb-4">Generate 4 AI-tailored interview questions based on your resume</p>
              ...
              <div className="space-y-3">
                ...
                <button onClick={handleGenerateInterviews} disabled={loadingInterviews}
                  ...
                  {loadingInterviews ? 'Generating...' : 'Generate 4 Questions'}
                </button>
              </div>
            )}
          </div>
      ...
      <Section title={`Career Roadmap ${isPro ? '' : '— Pro Feature'}`} icon={<Map size={16} className="text-blue-400" />} defaultOpen={false}>
        {!isPro ? (
          <div className="text-center py-6">
            <Lock size={28} className="text-zinc-600 mx-auto mb-3" />
            <p className="text-zinc-400 text-sm mb-1">Career roadmap requires <strong className="text-white">Pro plan</strong></p>
            <p className="text-zinc-500 text-xs mb-4">Get a step-by-step AI plan from Beginner to Advanced for your target role</p>
          ...
          <div className="space-y-4">
            ...
            {roadmap.gap_analysis && (
              <p className="text-zinc-400 text-sm p-3 bg-white/[0.02] rounded-xl">
                <span className="text-white font-medium">Gap Analysis: </span>{roadmap.gap_analysis}
              </p>
            )}

            {roadmap.top_skills_needed?.length > 0 && (
              <div className="p-3 bg-indigo-500/8 border border-indigo-500/15 rounded-xl">
                <p className="text-indigo-400 text-xs font-semibold mb-2">🎯 Top Skills Needed</p>
                <div className="flex flex-wrap gap-2">
                  {roadmap.top_skills_needed.map((s: string) => (
                    <span key={s} className="px-2 py-0.5 bg-indigo-500/12 text-indigo-300 text-xs rounded-md">{s}</span>
                  ))}
                </div>
              </div>
            )}
                ...
                <div key={m.phase} className="p-4 glass rounded-xl">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {m.level && (
                      <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${
                        m.level === 'Beginner' ? 'bg-emerald-500/15 text-emerald-400' :
                        m.level === 'Intermediate' ? 'bg-yellow-500/15 text-yellow-400' :
                  ...
                  {m.skills_to_learn?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {m.skills_to_learn.map((s: string) => (
                        <span key={s} className="px-2 py-0.5 bg-blue-500/12 text-blue-300 text-xs rounded-md">{s}</span>
                      ))}
                    </div>
                  )}
                  {m.actions?.length > 0 && (
                    <ul className="text-zinc-400 text-xs space-y-0.5 mb-2">
                      {m.actions.slice(0,3).map((a: string, i: number) => <li key={i}>• {a}</li>)}
                    </ul>
                  )}
                  {m.resources?.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {m.resources.map((r: any) => (
                        <a key={r.name} href={typeof r === 'string' ? '#' : r.url} target="_blank" rel="noopener noreferrer"
            ...
            {roadmap.certifications?.length > 0 && (
              <div className="p-3 bg-yellow-500/8 border border-yellow-500/15 rounded-xl">
                ...
                <div className="flex flex-wrap gap-2">
                  {roadmap.certifications.map((c: string) => (
                    <span key={c} className="px-2 py-0.5 bg-yellow-500/12 text-yellow-300 text-xs rounded-md">{c}</span>
                  ))}
                </div>
              </div>
            )}

            {roadmap.key_companies?.length > 0 && (
              <div className="p-3 bg-white/[0.02] border border-white/8 rounded-xl">
                <p className="text-zinc-400 text-xs font-semibold mb-2">🏢 Top Companies Hiring</p>
                <div className="flex flex-wrap gap-2">
                  {roadmap.key_companies.map((c: string) => (
                    <span key={c} className="px-2 py-0.5 bg-white/5 text-zinc-300 text-xs rounded-md">{c}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div>
            {!showRoadmapInput ? (
              <div className="text-center py-4">
                <p className="text-zinc-400 text-sm mb-4">Get a personalized AI roadmap from Beginner to Advanced for your dream role</p>
              ...
              <div className="space-y-3">
                <input value={targetRole} onChange={e => setTargetRole(e.target.value)}
                  placeholder="e.g. Python Developer, Data Scientist, Full Stack Developer..."
